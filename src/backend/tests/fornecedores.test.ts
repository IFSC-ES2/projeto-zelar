...
jest.mock('../src/services/FornecedorService');
...
describe('Testes Essenciais Fornecedor', () => {
  ...
  describe('POST /api/fornecedores', () => {
    it('Deve criar um fornecedor com dados válidos', async () => {
      const mockResult = { id: 1, razao_social: 'Empresa XPTO', cnpj_cpf: '123' };
      (MockedService.prototype.create as jest.Mock).mockResolvedValue(mockResult);

      const res = await request(app)
        .post('/api/fornecedores')
        .send({ razao_social: 'Empresa XPTO', cnpj_cpf: '123' });
    ...
    it('Deve retornar 400 se faltar campos obrigatórios', async () => {
      const res = await request(app)
        .post('/api/fornecedores')
        .send({ nome_fantasia: 'Apenas Fantasia' }); // Falta razão social e cnpj
  ...
  describe('DELETE /api/fornecedores/:id', () => {
    it('Deve retornar 409 se houver patrimônios vinculados', async () => {
